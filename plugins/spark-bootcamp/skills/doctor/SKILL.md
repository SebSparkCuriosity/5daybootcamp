...
**What this does.** Verifies your machine has the tools the week needs, builds your empty project skeleton, and writes a pre-work checklist for before Monday.
**Why it matters.** Two bits of pre-work take days, not minutes (a bank account that can receive money and a trading entity), and you take a real payment on Friday, so Doctor surfaces them now while there is still time.
...
5. Walk them through `.spark/prework.md`. Land the two slow items hard: the business bank account (one to three weeks in Jersey) and the trading entity. Start those today.
...
- The skeleton: `.spark/`, `.spark/brand/`, `.spark/deliverables/`, the five day folders (`01-discovery` through `05-sale`), and empty-with-header `CHANGELOG.md` and `DECISIONS.md`.
- `.spark/prework.md`: the Sunday checklist, copied from this skill's `references/prework.md`.

It does not create `.spark/state.json` (that belongs to `start`). The report notes this with a `[SKIP]` line.

## Done when
The summary reads `Needs a fix: 0` (all pass, or every unresolved item is a warning consciously skipped for the path), the eight folders and two logs exist, and `.spark/prework.md` is written. Then print exactly this and nothing after:
```
...
If `bash` is missing on Windows, git working means Git Bash or WSL is present, so the script runs. If not, create the eight folders and two logs by hand, copy `references/prework.md` to `.spark/prework.md`, and note checks were manual.
